...
namespace Base
{
    public partial class XHShow : Form
    {
        ...
        //窗口加载
        private void XH_Load(object sender, EventArgs e)
        {
            this.Hide();
            ...
            //判断当前电脑是否启动数据库服务
            if (!ServiceIsRunning("MySQL", 21))
            {
                MessageBox.Show("Mysql服务启动失败，请重装数据库");
            }
        ...
        private void 工具ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                ToolSetWorkOrderMenuItem.Enabled = MyDevice.devSum > 0 ? true : false;
            }
        }
        ...
        private string GetMysqlPath()
        {
            ...
            if (Directory.Exists(@"D:\Program Files\MySql\mysql-5.7.13-winx64"))
            {
                mysqlPath = @"D:\Program Files\MySql\mysql-5.7.13-winx64";
            }
            else
            {
                if (Directory.Exists(@"C:\Program Files\MySql\mysql-5.7.13-winx64"))
                {
                    mysqlPath = @"C:\Program Files\MySql\mysql-5.7.13-winx64";
                }
                else
                {
                    MessageBox.Show("请安装本软件对应的数据库，否则无法正常使用软件部分功能！");
                }
            }
            return mysqlPath;
        }
        ...
        //保存mysql数据库路径
        private void SaveMysqlPath()
        {
            string savePath = MyDevice.userDAT + @"\sqlPath.txt";

            string sqlPath = string.Empty;

            sqlPath = GetMysqlPath();
            ...
            System.IO.File.WriteAllText(savePath, sqlPath);
            System.IO.File.SetAttributes(savePath, FileAttributes.ReadOnly);//设置文件权限只读
        }


        //判断某个Windows服务是否处于运行状态
        private bool ServiceIsRunning(string serviceName, int serviceNameMaxLength)
        {
            bool result = false;
